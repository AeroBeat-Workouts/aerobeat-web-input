...
The public 4x3 and 8x6 contracts use athlete-space top-left row-major IDs. A small normalized hysteresis margin suppresses boundary jitter. Nose transitions retain dominant-axis cardinal evidence for existing defensive diagnostics. Wrist transitions use a bounded rolling linear fit over wrist-minus-corresponding-shoulder positions in square grid-cell units, then quantize the smoothed vector to `up`, `up-right`, `right`, `down-right`, `down`, `down-left`, `left`, or `up-left`. Motion below the bounded minimum magnitude is ambiguous and emits no directional entry; outside-to-grid movement emits no synthetic evidence. Rolling wrist history is measured-only and clears on tracking/calibration/source identity invalidation, rollback, and teardown.
